...
namespace PingaUnitBooking.UI.Controllers
{
    ...
    public class MailConfigureController : Controller
    {
        ...
        public async Task<IActionResult> TestMailConfigure([FromQuery] int MailConfigureID, string Type, string ToEmail, string ToMobileNo, string TemplateID, string Message)
        {
            try
            {
                var GroupID = _ld.GroupID;
                var responseData = await notificationService.TestMailConfigure(GroupID, MailConfigureID, Type, ToEmail, ToMobileNo, TemplateID, Message);
                if (!responseData.IsSuccess)
                {
                    return Json(new { success = false, message = responseData.Message });
                }
